...
class MatchActivityTest{
    ...
    fun testAppBehaviour() {
        ...
        // Apakah ada text Norwich? Kalo ada klik
        onView(withText("Norwich"))
                .check(matches(isDisplayed()))
        onView(withText("Norwich")).perform(click())

        Thread.sleep(wait)

        // Apakah ada text Norwich
        onView(withText("Norwich")).check(matches(isDisplayed()))
